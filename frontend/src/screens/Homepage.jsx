...
function Homepage() {
  ...
  const filteredSharers = sharers.filter(sharer =>
        ...
        {filteredSharers.map(sharer => (
          <div className="col-md-4 mb-3 d-flex" key={sharer.id}>
            <Card style={{ width: '18rem' }}>
              {sharer.image && (
                <Card.Img variant="top" src={sharer.image} style={{ width: '100%', height: 'auto' }} />
              )}
              <Card.Body className="p-3 d-flex flex-column justify-content-between">
                <div>
                  <Card.Title>{sharer.name}</Card.Title>
                  <Card.Text>{sharer.description}</Card.Text>
                  <Card.Text><small className="text-muted">{sharer.category}</small></Card.Text>
                </div>
